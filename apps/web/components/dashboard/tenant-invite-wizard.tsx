...
export function TenantInviteWizard({
  ...
  return (
    <ModalOverlay open={open} onClose={() => onOpenChange(false)}>
      <div className="w-full max-w-3xl rounded-[28px] border border-border bg-card p-6 shadow-2xl sm:p-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            {step === 3 ? (
              <>
                ...
                <Button type="button" onClick={handleCloseToDashboard} title="Close the wizard and return to the dashboard.">
                  Back to Dashboard
                </Button>
              </>
            ) : (
              <>
                <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} title="Close the tenant invite wizard.">
                  Close
                </Button>
                <Button type="button" disabled={isPending} onClick={handleNext} title={step === 2 ? "Send the invitation." : "Continue to the next step."}>
