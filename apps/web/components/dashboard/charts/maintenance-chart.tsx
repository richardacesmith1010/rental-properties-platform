...
export function MaintenanceChart({ metrics }: MaintenanceChartProps) {
  if (metrics.length === 0) {
    return <div className="flex h-[250px] items-center justify-center text-sm text-[var(--muted)]">No maintenance activity yet.</div>;
  }
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} layout="vertical" margin={{ left: 8, right: 24, top: 8, bottom: 0 }}>
          ...
          <Bar
            dataKey="resolved"
            stackId="tickets"
            fill="color-mix(in srgb, var(--accent) 78%, var(--surface))"
            name="Resolved"
            radius={[0, 4, 4, 0]}
          />
          <Bar
            dataKey="unresolved"
            stackId="tickets"
            fill="color-mix(in srgb, var(--accent) 26%, var(--surface-3))"
            name="Unresolved"
            radius={[0, 4, 4, 0]}
          >
