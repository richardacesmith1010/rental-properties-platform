...
export interface PropertyFileAuthorizationInput extends BaseAuthorizationInput {
  visibility: "owner_manager" | "all";
  tenantHasLease: boolean;
}
...
export function canAccessPropertyFile(input: PropertyFileAuthorizationInput): boolean {
  if (input.role === "owner") {
    return Boolean(input.isPropertyAdmin || input.propertyOwnerId === input.userId);
  }
  ...
  return input.visibility === "all" && input.tenantHasLease;
}
