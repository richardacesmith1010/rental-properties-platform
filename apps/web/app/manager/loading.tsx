      ...
      <div className="hidden w-64 flex-shrink-0 lg:block">
        <div className="gradient-sidebar h-full space-y-4 p-4">
          ...
          <div className="space-y-2">
            {Array.from({ length: 7 }).map((_, i) => <div key={i} className="domus-skeleton h-9 w-full rounded-lg" />)}
          </div>
        </div>
      </div>
      <div className="flex-1 space-y-6 p-6">
        <div className="space-y-2">
          <div className="domus-skeleton h-8 w-56" />
          <div className="domus-skeleton h-4 w-44" />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => <div key={i} className="domus-skeleton h-20 rounded-2xl" />)}
        </div>
